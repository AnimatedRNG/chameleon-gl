...
InputController::InputController(SDL::window_params* wp) :
    keypress_map(),
...
glm::mat4 InputController::get_model() {
    return model;
}

glm::mat4 InputController::get_view() {
    return view;
}

glm::mat4 InputController::get_projection() {
    return projection;
}

void InputController::set_aspect_ratio(const float& ratio) {
    this->aspect_ratio = ratio;
}

void InputController::on_event(SDL_Event& event) {
    switch (event.type) {
        case SDL_KEYDOWN:
